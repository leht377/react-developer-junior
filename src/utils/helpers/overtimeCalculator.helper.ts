...
const calcularHED = (scheduleType: ScheduleType, employe: EmployeeEntity): number => {
  ...
  for (const control of accessControls) {
    const { check_in, check_out } = control.attributes

    const day = check_in.getDay()
    if (day === DAYS_OF_WEEK_NAMES.SUNDAY || isHoliday(check_in)) continue
...
const calcularHEN = (scheduleType: ScheduleType, employe: EmployeeEntity): number => {
  let horasExtrasSegundos = 0
...
const calcularHEDDominical = (scheduleType: ScheduleType, employe: EmployeeEntity): number => {
  let horasExtrasSegundos = 0
...
const calcularHENDominical = (scheduleType: ScheduleType, employe: EmployeeEntity): number => {
  let horasExtrasSegundos = 0
